...
// 以下两个宏按照实际情况选择
#define STDIO_REDIRECT_PERIPHERALS USB // UART or USB
#define STDIO_REDIRECT_COMPILER GCC     // GCC or MDK

// 根据选择的外设和编译器选择对应的头文件
#if (STDIO_REDIRECT_PERIPHERALS == UART)
#include "usart.h"
#define STDIO_REDIRECT_UART &huart1
#elif (STDIO_REDIRECT_PERIPHERALS == USB)
#include "usbd_cdc_if.h"
#else
#error "Please select the peripherals for stdio redirect."
#endif /* STDIO_REDIRECT_PERIPHERALS */

#if (STDIO_REDIRECT_COMPILER == MDK)
#include "RTE_Components.h"
#endif /* STDIO_REDIRECT_COMPILER */

#if (STDIO_REDIRECT_PERIPHERALS == USB)
/**
 * @brief  通过 USB 发送数据
...
#elif (STDIO_REDIRECT_COMPILER == GCC)
